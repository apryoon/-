...
export default async function handler(req, res) {
  ...
  const { password, data, mouData = [], mouUpdateDate = '' } = req.body || {};
  ...
  try {
    ...
    const payload = {
      data,
      mouData: Array.isArray(mouData) ? mouData : [],
      rowCount: data.length,
      mouCount: Array.isArray(mouData) ? mouData.length : 0,
      updatedAt: timestamp,
